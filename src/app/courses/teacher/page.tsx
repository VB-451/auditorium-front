...
export default async function HomeTeacherServer() {
    ...
    const postsData = await fetchPostsBy(undefined, cookieStore.get("userID")?.value, cookieStore.get("accessToken")?.value)
    ...
    return (
    <>
      <CoursesPage data={coursesData} posts={postsData} type="teacher" />
    </>
  );
}
