...
export default function Enroll(){
    ...
    return (
        <form onSubmit={handleEnroll} className="bg-white w-96 h-64 py-4 px-4 rounded-xl flex flex-col items-center">
            ...
            onChange={handleKeyChange} value={joinKey}/>
            <p className={`${(keyValid || joinKey.length === 0) ? "text-transparent" : "text-primary_pink"} mt-1`}>Key is not valid</p>
            <button disabled={!keyValid || !joinKey} onClick={handleEnroll} className={`text-white text-xl font-semibold w-5/12 h-12 mt-3 rounded 
